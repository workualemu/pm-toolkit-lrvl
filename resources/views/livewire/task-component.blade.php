    ...
    <div class="border-b border-slate-150 py-3 dark:border-navy-500" wire:click="openModal({{ $task->id }})">
        ...
        <div class="mt-1 flex items-end justify-between">
            <div class="flex flex-wrap items-center font-inter text-xs">
                ...
                <a href="#"
                    x-data="{ }"
                    @click.stop="$wire.filterByStatus({{ $task->taskStatus?->id }});"
                >
                    <span class="flex items-center space-x-1 text-{{$task->taskStatus?->color}}-700">
                ...
                <div class="flex items-center space-x-2 text-xs text-slate-400 dark:text-navy-300">
                    @foreach($task->tags as $tag)
                        <div class="bg-{{$tag->color}}-600 badge text-white">{{$tag->label}}</div>
                    @endforeach
                </div>
            </div>
            @if($task->level == 2)
