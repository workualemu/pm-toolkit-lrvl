<main class="main-content kanban-app w-full">
    ...
    @else
        <div class="" >
            <x-search-header title="Project Users" subtitle="{{ __('For project: ') }} {{ $project->title }}" searchModel="searchTerm" />
            <div class="py-4"></div>
            ...
            <div class="items-center border-t border-slate-150 py-3 dark:border-navy-600">
                <div class="px-2 flex items-center justify-between dark:border-navy-600">
                    <x-button color="error" wire:click="assignUsers(false)">
                        {{ __('Close') }}
                    </x-button>
                    <x-button color="info" wire:click="assignUsers(true)">
                        {{ __('Save') }}
                    </x-button>
                </div>
            </div>
